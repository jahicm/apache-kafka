package org.kafka.stream.utils.datagen;
...
public class DataGenerator {
	...
	public static List<Purchase> generatePurchases(int number, int numberCustomers) {
		...
		for (int i = 0; i < number; i++) {
			...
			Purchase purchase = Purchase.builder().creditCardNumber(customer.creditCardNumber)
					.customerId(customer.customerId).department(store.department).employeeId(store.employeeId)
					.firstName(customer.firstName).lastName(customer.lastName).itemPurchased(itemPurchased)
					.quanity(quantity).price(price).purchaseDate(purchaseDate).zipCode(store.zipCode)
					.storeId(store.storeId).build();
	...
	/**
	 * Special method for generating customers with static customer ID's so we can
	 * easily run interactive queries with a predictable list of names
	 * 
	 * @return customer list
	 */
	public static List<Customer> generateCustomersForInteractiveQueries() {
		List<Customer> customers = new ArrayList<>(10);
		List<String> customerIds = Arrays.asList("12345678", "222333444", "33311111", "55556666", "4488990011",
				"77777799", "111188886", "98765432", "665552228", "660309116");
		Faker faker = new Faker();
	...
	private static Purchase generateCafePurchase(Purchase purchase, Faker faker) {
		...
		return Purchase.builder(purchase).department("Coffee")
				...
				.price(Double.parseDouble(faker.commerce().price(3.00, 6.00))).quanity(1).purchaseDate(cafeDate)
				.build();
